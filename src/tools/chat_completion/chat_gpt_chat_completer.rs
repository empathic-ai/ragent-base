...
use openai_api_rs::v1::{chat_completion::*, common::*, types::*};
use tracing::info;
...
impl ChatCompleter for ChatGPTChatCompleter {
    async fn get_response(
                ...
                openai_api_rs::v1::chat_completion::ChatCompletionMessage {
                    role: match message.role {
                        ...
                        super::MessageRole::function => {
                            openai_api_rs::v1::chat_completion::MessageRole::function
                        }
                    },
                    content: match message.content {
                        ...
                        super::Content::ImageUrl(image_url) => {
                            openai_api_rs::v1::chat_completion::Content::ImageUrl(vec![
                                openai_api_rs::v1::chat_completion::ImageUrl {
                                    r#type:
                                        openai_api_rs::v1::chat_completion::ContentType::image_url,
                                    text: None,
                                    image_url: Some(
                                        openai_api_rs::v1::chat_completion::ImageUrlType {
                                            url: image_url[0].image_url.clone().unwrap().url,
                                        },
                                    ),
                                },
                            ])
        ...
        let mut stream = client
            .chat_completion_stream(chat_completion_request.clone())
            .await
            .map_err(|x| anyhow!("Failed to get chat completion stream: {}", x))?;
            ...
            match x {
                ...
                ChatCompletionStreamResponse::Usage(usage) => {
                    ...
                    Ok(super::ChatCompletionResponse {
                        ...
                        estimated_cost: cost,
                        usage_quantity: (usage.prompt_tokens + usage.completion_tokens) as f32,
                        usage_unit: "tokens".to_string(),
                    })
                }

                ChatCompletionStreamResponse::Done => Ok(super::ChatCompletionResponse {
                    completion: String::new(),
                    estimated_cost: Decimal::ZERO,
                    usage_quantity: 0.0,
                    usage_unit: String::new(),
                ...
                _ => Ok(super::ChatCompletionResponse {
                    completion: String::new(),
                    estimated_cost: Decimal::ZERO,
                    usage_quantity: 0.0,
                    usage_unit: String::new(),
...
fn calculate_cost(model: &str, usage: &openai_api_rs::v1::common::Usage) -> Result<Decimal> {
    ...
    let (input_price, cached_input_price, output_price) = match model {
        ...
        GPT4_O_MINI | GPT4_O_MINI_2024_07_18 => (dec!(0.15), dec!(0.075), dec!(0.60)),

        GPT4_1 | GPT4_1_2025_04_14 => (dec!(2.00), dec!(0.50), dec!(8.00)),

        GPT4_1_MINI | GPT4_1_MINI_2025_04_14 => (dec!(0.40), dec!(0.10), dec!(1.60)),

        GPT4_1_NANO | GPT4_1_NANO_2025_04_14 => (dec!(0.10), dec!(0.025), dec!(0.40)),

        GPT5 | GPT5_2025_08_07 | GPT5_CHAT_LATEST | GPT5_CODEX => {
            (dec!(1.25), dec!(0.125), dec!(10.00))
        }

        GPT5_MINI | GPT5_MINI_2025_08_07 => (dec!(0.25), dec!(0.025), dec!(2.00)),

        GPT5_NANO | GPT5_NANO_2025_08_07 => (dec!(0.05), dec!(0.005), dec!(0.40)),

        GPT5_PRO | GPT5_PRO_2025_10_06 => (dec!(15.00), dec!(15.00), dec!(120.00)),

        O1 | O1_2024_12_17 => (dec!(15.00), dec!(7.50), dec!(60.00)),

        O1_PRO | O1_PRO_2025_03_19 => (dec!(150.00), dec!(150.00), dec!(600.00)),

        O3 | O3_2025_04_16 => (dec!(2.00), dec!(0.50), dec!(8.00)),
    ...
    let uncached_input_cost = Decimal::from(uncached_tokens) / million * input_price;

    let cached_input_cost = Decimal::from(cached_tokens) / million * cached_input_price;

    let output_cost = Decimal::from(usage.completion_tokens) / million * output_price;

    Ok(uncached_input_cost + cached_input_cost + output_cost)
}
