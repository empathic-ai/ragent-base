...
use openai_api_rs::realtime::client_event::{
    ConversationItemCreate, InputAudioBufferAppend, ResponseCreate,
};
    ...
    pub async fn new(api_key: String) -> Self {
            ...
            while let Ok(ev) = &mut input_rx.recv().await {
                match ev {
                    RealtimeEvent::Audio(samples) => {
                        ...
                        let mut samples =
                            AudioClip::new(AudioFormat::new(16000, 1, 16), samples.clone())
                                .resample(AudioFormat::new(24000, 1, 16))
                                .unwrap()
                                .samples;
                        ...
                        let item_create_message: Message = ConversationItemCreate {
                            item: Item {
                                ...
                                content: Some(vec![ItemContent {
                                    r#type:
                                        openai_api_rs::realtime::types::ItemContentType::InputText,
        ...
        tokio::task::spawn(async move {
            while let Some(message) = read.next().await {
                ...
                match message {
                    Message::Text(text) => {
                        let server_event: ServerEvent = serde_json::de::from_str(&text)
                            .expect(&format!("Failed to deserialize server event: {}", text));
                        match server_event {
                            ServerEvent::ResponseAudioDelta(ev) => {
                                let bytes = base64::decode(ev.delta).unwrap();
                                let samples = AudioClip::new(
                                    AudioFormat::new(24000, 1, 16),
                                    convert_16_bit_u8_to_i16(&bytes).to_vec(),
                                )
                                .resample(AudioFormat::new(16000, 1, 16))
                                .unwrap()
                                .samples;
