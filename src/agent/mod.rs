use bevy::reflect::{DynamicStruct, Enum, EnumInfo, ReflectRef, Typed};
...
pub fn get_function_prompt(task_configs: Vec<TaskConfig>) -> String {
    let available_task_configs: Vec<TaskConfig> = task_configs
        .iter()
        .filter(|task_config| task_config.is_available)
        .cloned()
        .collect();
    let unavailable_task_configs: Vec<TaskConfig> = task_configs
        .iter()
        .filter(|task_config| !task_config.is_available)
        .cloned()
        .collect();
...
pub fn get_task_configs_description(task_configs: Vec<TaskConfig>) -> String {
    task_configs
        .iter()
        .map(|task_config| {
            let parameters_description: String = task_config
                .parameters
                .iter()
            ...
            format!(
                "{}({}) - {}",
                task_config.name, parameters_description, task_config.description
            )
        })
        .collect::<Vec<_>>()
        .join("\n\n")
...
pub trait Agent: Send + Sync {
    ...
    */
    ...
    fn wrap_speech_text(&self, speaker: String, speech_text: String) -> String {
        format!(
            r#"speak("{}")"#,
            speech_text.trim().trim_matches('\'').trim_matches('"')
        )
    }
    ...
    fn parse_partial_json(&self, json_str: &str) -> (bool, Result<Value>) {
        ...
        (
            open_braces == 0,
            serde_json::from_str(&well_formed_json).map_err(|err| anyhow!(err)),
        )
    }
